 ...
 * 개인정보처리방침 본문.
 *
 * **번역하지 않는다.** 법적 효력을 갖는 문서라 기계 번역본을 게시하면 어느 쪽이 구속력을
 * 갖는지가 불분명해진다(웹·iOS 와 같은 결정). 한국어가 아닌 표시 언어에서는 본문 위에
 * `language_koreanOnlyNotice` 안내를 붙이고 제목·내비게이션 라벨만 번역한다.
 *
 * 원문은 clipnote.co.kr/privacy 이고 iOS `PrivacyView` 와 **거의** 같다.
 *
 * ## iOS 원문과 일부러 다른 곳 (되돌리지 말 것)
 *
 * 광고 식별자 문단이다. iOS 원문은 "이용자 추적 권한(App Tracking Transparency)을 요청하지
 * 않으므로 광고 식별자(IDFA)는 수집하지 않습니다" 라고 적는데, ATT 도 IDFA 도 **안드로이드에는
 * 없는 개념이다.** 대응물은 Google 광고 ID(AAID) 이고, 이 앱은 매니페스트에 `AD_ID` 권한을
 * 선언하고 AdMob 이 실제로 그 값을 쓴다. 원문을 그대로 실으면 **사실과 다른 고지**가 된다.
 *
 * 그래서 세 곳을 고쳤다 — 1항 수집 항목 목록, 1항 광고 문단, 5항 제3자 제공.
 * 나머지는 문장 하나까지 웹·iOS 와 같아야 한다. **한쪽만 바꾸지 않는다.**
 */
object PrivacyContent {
    const val EFFECTIVE_DATE = "시행일: 2026년 8월 4일"

    const val INTRO = "ClipNote(이하 \"서비스\")는 「개인정보 보호법」을 준수하며, 이용자의 개인정보를 보호하기 위해 다음과 같이 개인정보처리방침을 두고 있습니다. 서비스는 회원 로그인에 필요한 최소한의 정보만 수집합니다."
    ...
    val SECTIONS: List<Section> = listOf(
        Section(
            ...
            listOf(
                ...
                "· 광고 게재 과정에서 자동 수집: 기기·브라우저 정보, IP 주소, 광고 ID(AAID), 광고 조회·클릭 기록",
                "비로그인 상태로 이용하는 경우, 저장한 클립과 태그는 서버로 전송되지 않고 이용자의 기기 내 저장소에만 보관됩니다.",
                "서비스는 무료로 제공하기 위해 광고를 게재합니다(모바일 앱 Google AdMob, 웹 Google AdSense). 광고 모듈이 동작하는 시점부터 Google LLC가 위 광고 관련 정보를 자동으로 수집하며, 실제 광고가 표시되지 않더라도 수집이 일어날 수 있습니다. 서비스는 이 정보를 자체 서버에 저장하지 않습니다. 안드로이드 앱은 광고 게재와 성과 측정을 위해 Google 광고 ID(AAID)를 사용하며, 이 식별자는 Google LLC가 수집합니다. 이용자는 기기의 [설정 > 개인정보 보호 > 광고]에서 광고 ID를 삭제하거나 맞춤 광고를 선택 해제할 수 있습니다. 서비스는 그 밖의 기기 고유 식별자를 수집하지 않습니다.",
            ),
        ),
        Section(
            "2. 개인정보의 수집·이용 목적",
        ...
        Section(
            "5. 개인정보의 제3자 제공",
            listOf(
                "서비스는 이용자의 개인정보를 외부에 판매하지 않으며, 법령에 따라 요구되는 경우를 제외하고 제3자에게 제공하지 않습니다. 다만 광고 게재를 위해 위 4항의 광고 사업자에게 기기·브라우저 정보와 IP 주소, 광고 ID(AAID)가 전달됩니다. 이용자는 기기의 [설정 > 개인정보 보호 > 광고] 또는 Google 광고 설정(adssettings.google.com)에서 맞춤 광고를 제한할 수 있습니다.",
            ),
        ),
        Section(
            "6. 이용자의 권리와 행사 방법",
